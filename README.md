...
1. API (src/components/api.ts) - Работа с сервером
   ...
   **Конструктор**:
   ...
   Принимает базовый URL сервера (https://larek-api.nomoreparties.co) и объект с дополнительными настройками запросов (заголовки, методы и т.п.). Сохраняет их в соответствующие поля.
   **Поля**:
   ...
   **Методы**:
   ...
   - handleResponse(response: Response): Promise<object> — проверяет ответ от сервера. Возвращает Promise с данными или ошибкой, если статус ответа не OK.
...
2. EVENTS (src/components/events.ts) - События между частями приложения
   ...
   **Поля**:
   ...
   - events: Map<string | RegExp, Set Fn> — коллекция всех подписок на события. Ключ — имя события (или шаблон), значение — множество функций-обработчиков.

   **Методы**:

   - on(eventName: string | RegExp, callback: (data?: object) => void): void
     Подписывает обработчик на событие или шаблон события.
   - emit(eventName: string, data?: object): void
     Вызывает событие с именем `eventName` и передаёт данные всем функциям-обработчикам, зарегистрированным на это событие.
   - off(eventName: string | RegExp, callback: Function): void
     Убирает обработчик с события или шаблона.
   - onAll(callback: (event: { eventName: string; data?: object }) => void): void
     Подписывает обработчик на все события.
   - offAll(): void
     Удаляет всех подписчиков со всех событий.
   - trigger(eventName: string, context?: object): (data: object) => void
     Возвращает функцию, которая вызывает событие с объединением переданных данных с контекстом.
